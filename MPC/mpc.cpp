...
bool initMat(OsqpEigen::Solver &solver)
{
    ...
    Eigen::VectorXd gradient = (F * xk).transpose(); // 将结果存储在临时变量中
    // std::cout << "Here is the matrix gradient:\n" << gradient << std::endl;

    Eigen::SparseMatrix<double> LinearConstraintsMat = Gamma.sparseView();
    // std::cout << "Here is the matrix H_sparse:\n" << H_sparse << std::endl;

    //
    Eigen::VectorXd upperBound(Np * n);
    for (int i = 0; i < Np; i++)
    {
        upperBound(i * n) = x_high(0) - (Fai * xk)(i, 0);
        upperBound(i * n + 1) = x_high(1);
    }
    ...
    for (int i = 0; i < Np; i++)
    {
        lowerBound(i * n) = x_low(0) - (Fai * xk)(i, 0);
        lowerBound(i * n + 1) = x_low(1);
    }
    //std::cout << "Here is the matrix lowerBound:\n" << lowerBound << std::endl;
    ...
    if (!solver.data()->setLinearConstraintsMatrix(LinearConstraintsMat))
    {
        return false;
    }
